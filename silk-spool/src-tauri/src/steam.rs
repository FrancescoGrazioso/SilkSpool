...
mod tests {
  ...
  fn test_silksong_patterns() {
    ...
    let hollow_knight_spaced_pattern =
      regex::Regex::new(r"(?i)hollow\s+knight\s+[:]?\s*silksong").unwrap();

    // Test silksong pattern against all cases
    for case in &test_cases {
      assert!(
        silksong_pattern.is_match(case),
        "Pattern 'silksong' should match '{}'",
        case
      );
    }
    ...
    for case in &full_name_cases {
      assert!(
        hollow_knight_pattern.is_match(case),
        "Pattern 'hollow.?knight.?silksong' should match '{}'",
        case
      );
      // Skip the spaced pattern test for cases with colons
      if !case.contains(':') {
        assert!(
          hollow_knight_spaced_pattern.is_match(case),
          "Pattern 'hollow knight silksong' should match '{}'",
          case
        );
      }
