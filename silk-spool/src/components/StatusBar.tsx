...
export const StatusBar: React.FC<StatusBarProps> = ({ gameStatus, onSelectPath }) => {
  ...
  return (
    <div className='flex items-center space-x-4'>
      ...
      <div className='flex items-center space-x-2'>
        ...
        <span className='text-sm text-gray-300'>{getGameStatusText()}</span>
      ...
      <div className='flex items-center space-x-2'>
        ...
        <span className='text-sm text-gray-300'>{getBepInExStatusText()}</span>
        {gameStatus.bepinex.present && !gameStatus.bepinex.initialized && (
          <span className='text-xs text-yellow-400'>(Not initialized)</span>
        )}
      </div>
      ...
      {gameStatus.found && gameStatus.path && (
        <div className='flex items-center space-x-2'>
          ...
          <span className='text-xs text-gray-400 max-w-xs truncate'>{gameStatus.path}</span>
        </div>
      )}
    </div>
  );
